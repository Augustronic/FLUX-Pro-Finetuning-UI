    ...
    def check_progress(self, finetune_id: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                # If task not found but details exist, training is complete
            ...
            combined_result = {
                ...
                'is_completed': (
                    status_result.get('status') in ['Ready', 'Task not found']
                )
            }
...
def monitor_progress(api_key: str, finetune_id: str, check_interval: int = 30):
    ...
    try:
        ...
        if finetune_id not in finetunes.get('finetunes', []):
            print(
                f"\nNote: Fine-tune ID {finetune_id} not found in your "
                "fine-tunes list."
            )
            print(
                "This is normal for newly started jobs. "
    ...
    while True:
        try:
            ...
            if status == 'Ready':
                ...
                msg = (
                    "\nStatus is Error - job may still be "
                    "initializing or in queue"
                )
                print(msg)
            elif status == 'Pending':
...
if __name__ == "__main__":
    ...
    FINETUNE_ID = "92258091-db89-4cf7-97c8-ece585d94231"  # Latest fine-tune ID
